...
const Home = ({ navigation }) => {
  ...
  function handleChangeCategory(categoryId, menuTypeId) {
    // Retrieve the popualr menu
    let selectedPopular = dummyData.menu.find((a) => a.name == "Popular");

    // Retrieve the recommended menu
    let selectedRecommned = dummyData.menu.find((a) => a.name == "Recommended");
    ...
    setRecommends(
      selectedRecommned?.list.filter((a) => a.categories.includes(categoryId))
    );

    // Set the menu based categoryId
    setMenuList(
  ...
  function renderPopularSection() {
    return (
      <Section
        ...
        onPress={() => console.log("Show all popualer items")}
      >
        <FlatList
          data={popular}
          keyExtractor={(item) => `${item.id}`}
